# GitHub and Overleaf Information

This file records where the ALEPH cumulant analysis code and note are maintained.

## GitHub Code Repository

- Web URL: https://github.com/yenjie/ALEPH_Cumulant
- SSH URL: `git@github.com:yenjie/ALEPH_Cumulant.git`
- Main branch: `main`
- Latest recorded code update: 2026-06-17, figure x-axis labels changed to `N_{trk}^{offline}` with larger title offsets

Clone with SSH:

```bash
git clone git@github.com:yenjie/ALEPH_Cumulant.git
cd ALEPH_Cumulant
```

If SSH is not configured, clone with HTTPS:

```bash
git clone https://github.com/yenjie/ALEPH_Cumulant.git
cd ALEPH_Cumulant
```

The detailed run instructions are in `docs/run_instructions.md`.

## Overleaf Analysis Note

- Web URL: https://www.overleaf.com/project/6a320514b63e8500bc0db2d7
- Git URL: `https://git@git.overleaf.com/6a320514b63e8500bc0db2d7`
- SSH Git URL, if port 22 is available: `git@git.overleaf.com:6a320514b63e8500bc0db2d7`
- Main branch: `main`
- Latest recorded note update: 2026-06-17, figure x-axis labels changed to `N_{trk}^{offline}` with larger title offsets

The Overleaf project contains the analysis note source, status report slides, selected figure PDFs/CSVs, and review markdown files. Token values are not stored in this repository.

## Current Analysis Artifacts

- Analysis note PDF in GitHub checkout: `AnalysisNote/main.pdf` (18 pages after the merged LEP1 update)
- Status report slides in GitHub checkout: `status_report_slides.pdf` and `status_report_slides.tex`
- Note figures in GitHub checkout: `AnalysisNote/figures/`, including inclusive, MC-only, data/MC, `|Delta eta| > 2.0`, `|Delta eta| > 1.6`, and two-subevent comparisons
- Review files in GitHub checkout:
  - `20260617-review.md`
  - `20260617-review-response.md`
- Review files in Overleaf checkout:
  - `review/20260617-review.md`
  - `review/20260617-review-response.md`

