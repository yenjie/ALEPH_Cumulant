...
SEED=${SEED:-12345}
OUTPUT=${OUTPUT:-output/pythia_noshoving_zpole_3M.root}
LOG=${LOG:-output/pythia_noshoving_zpole_3M.log}

mkdir -p "$(dirname "${OUTPUT}")" "$(dirname "${LOG}")"

make pythia PYTHIA8_CONFIG="${PYTHIA8_CONFIG}"

bin/generate_pythia_zpole_root   --Output "${OUTPUT}"   --Events "${EVENTS}"   --Seed "${SEED}"   --PythiaData "${PYTHIA8DATA}"   --EnableShoving 0   --ApplyAlephLikeAcceptance 0   --ReportEvery 50000   2>&1 | tee "${LOG}"
