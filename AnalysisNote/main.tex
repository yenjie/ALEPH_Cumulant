...
uncertainties are not yet included. Matched 1994 reconstructed-MC comparisons
are included as baseline closure-style checks of the beam-axis and thrust-axis
trends, including a separate two-subevent cumulant construction.
\end{abstract}
...
The figures in this note currently use the matched LEP1 1994 reconstructed
data sample
\begin{center}
...
\end{center}
This file contains \(1{,}365{,}440\) entries in the tree \texttt{t}; after the
charged-particle selection and multiplicity-bin requirement, \(1{,}321{,}026\)
events enter the main cumulant histograms. The input format is the vector
layout with \texttt{px}, \texttt{py}, \texttt{pz}, \texttt{pwflag}, and
optional \texttt{passEventSelection} branches.

This is not yet the full 1991--1995 LEP1 data set. The available local LEP1
path lists contain 15, 31, 27, 171, and 31 input files for 1991, 1992, 1993,
1994, and 1995, respectively. A combined vector-format input, or an equivalent
year-by-year chunk production merged at the histogram level, is needed before
the result can be quoted as using all LEP1 statistics. The older local
StudyMult fixed-array file
\path{/data/yjlee/StudyMult/DataFiles/ROOTfiles/cleaned_ALEPH_Data-all.aleph.root}
contains \(59{,}874\) entries and corresponds to an earlier prototype
production; those counts are not used for the current figures.
...
sixteen-chunk jackknife procedure. The processed inputs contain 771,597 MC
entries and 1,365,440 data entries; after the charged-particle selection and
multiplicity-bin requirement, 749,082 MC events and 1,321,026 data events enter
the inclusive cumulant histograms.
...
the matched 1994 data and reconstructed-MC samples were reprocessed with an
...
same as in the inclusive data/MC comparison. The only additional requirement is
a strict pair cut
\begin{equation}
  |\eta_i-\eta_j| > 2.0,
\end{equation}
where \(\eta\) is evaluated in the same coordinate system as the azimuthal angle:
...
ratio falls to about 0.66 in the 30--35 bin, with the high-multiplicity tail
...
ratio decreases from about 0.74 in the 0--10 bin to about 0.44 around 30--35
...
\begin{figure}[htbp]
  ...
  \(\vn{2}{2,|\Delta\eta|>2}\) for matched LEP1 1994 data and reconstructed
...
\subsection{Two-subevent matched data/MC comparison}
\label{subsec:two-subevent-comparison}

The matched 1994 data and reconstructed-MC samples were reprocessed with the
...
\begin{figure}[htbp]
  ...
  \caption{Beam-axis two-subevent comparison of matched LEP1 1994 data and
  reconstructed MC. MC is drawn as a histogram with statistical band, while data
...
\begin{figure}[htbp]
  ...
  \caption{Thrust-axis two-subevent comparison of matched LEP1 1994 data and
  reconstructed MC. Missing or zero high-order entries correspond to
...
sums. The current figures use the matched LEP1 1994 reconstructed data sample,
with \(1{,}365{,}440\) input entries and \(1{,}321{,}026\) selected events in the
analysis multiplicity bins; this is not yet the full 1991--1995 LEP1 data
statistics. The analysis extracts \(\vn{2}{2}\), \(\vn{2}{4}\), \(\vn{2}{6}\),
and \(\vn{2}{8}\) versus charged-particle multiplicity for both the beam-axis
and thrust-axis azimuthal definitions. Statistical uncertainties are propagated
through the nonlinear cumulant definitions with a sixteen-chunk jackknife. The
beam-axis and thrust-axis figures included in this note provide the current
detector-level baseline for subsequent correction, all-LEP1 production, and
